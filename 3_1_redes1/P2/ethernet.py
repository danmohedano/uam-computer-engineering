...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    # Comprobar si la trama no va ni en difusión ni dirigida a nosotros
    if macdst != broadcastAddr and macdst != macAddress:
        return

    # Comprobar que hay una función asociada al Ethertype
    if upperProtos.get(ethertype) is None:
        return

    upperProtos[ethertype](us, header, data[14:], macsrc)
...
def startEthernetLevel(interface:str) -> int:
    ...
    handle = None
    # Comprobar si el nivel Ethernet ya estaba inicializado
    if levelInitialized:
        logging.info("Nivel ya inicializado.")
        return -1

    # Obtener la dirección MAC
    macAddress = getHwAddr(interface)

    # Abrir la interfaz especificada en modo promiscuo
    errbuf = bytearray()
    handle = pcap_open_live(interface, ETH_FRAME_MAX, PROMISC, TO_MS, errbuf)
    if not handle:
        logging.info("Error abriendo la interfaz.")
        return -1
...
def stopEthernetLevel()->int:
    ...
    '''
    if not recvThread:
        logging.info("Hilo no creado")
        return -1

    recvThread.stop()
    if not handle:
        logging.info("La interfaz ya estaba cerrada")
        return -1
...
def sendEthernetFrame(data:bytes,length:int,etherType:int,dstMac:bytes) -> int:
    '''
        ...
        Retorno: 0 si todo es correcto, -1 en otro caso
    '''
    global macAddress, handle, ETH_FRAME_MIN, ETH_FRAME_MAX
    if not handle:
        logging.info("Interfaz no abierta.")
        return -1

    if not macAddress:
        logging.info("Dirección MAC propia no inicializada.")
        return -1

    # Comprobar que el payload no es demasiado grande
    if length > ETH_FRAME_MAX - 14:
        logging.info("Tamaño de payload demasiado grande.(" + str(length) + ")")
        return -1

    # Generación de la cabecera Ethernet
    trama = bytes()
    trama += dstMac
    trama += macAddress
    ...
    # Agrandar la trama si no llega a los 60B
    while len(trama) < ETH_FRAME_MIN:
        trama += struct.pack("!B", 0)

    ret = pcap_inject(handle, trama, len(trama))

    if ret == 0:
        logging.info("No se pudo enviar la trama Ethernet.")
        return -1
    elif ret < len(trama):
        logging.info("La trama Ethernet no se envió completamente.")
        return -1
