...
class Clasificador:
    ...
    def matriz_confusion(self, particionado, dataset, seed=None):
        """Entrena el modelo con el particionado indicado y construye la matriz de confusión.
        ...
        """
        n_clases = len(set(dataset.datos.to_numpy()[:, -1]))
        ...
        for particion in particiones:
            ...
            for i in range(len(predicciones)):
                if predicciones[i] != -1:
                    confusion_matrix[(int(predicciones[i]) + 1) % 2][(int(datos_test[i][-1]) + 1) % 2] += 1
