...
class DatabaseAccess:
    ...
    def db_get_tabla1(self):
        """
        Función que obtiene la tabla con la información de aquellas películas
        que sean comedias de 1997 y que contengan la palabra "Life" en el título.
        :return: lista con la información de las películas
        """
        ...
        result = []
        cursor = collection.find({'year': '1997', 'genres': "Comedy",
                                  'title': {'$regex': 'Life', '$options': '$i'}})
        for item in cursor:
            result.append(item)
        return result

    def db_get_tabla2(self):
        """
        Función que obtiene la tabla con la información de aquellas películas
        que han sido dirigidas por Woody Allen en los 90
        :return: lista con la información de las películas
        """
        myclient = pymongo.MongoClient("mongodb://localhost:27017/")
        mydb = myclient["si1"]
    ...
    def db_get_tabla3(self):
        """
        Función que obtiene la tabla con la información de aquellas películas
        en las que Jim Parsons y Johnny Galecki han compartido reparto
        :return: lista con la información de las películas
        """
        myclient = pymongo.MongoClient("mongodb://localhost:27017/")
        mydb = myclient["si1"]
        ...
        result = []
        cursor = collection.find({'actors': {'$all': ['Parsons, Jim (II)',
                                                      'Galecki, Johnny']}})
        for item in cursor:
            result.append(item)
        return result
